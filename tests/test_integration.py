...
def test_palantir_reproducibility(processed_data):
    ...
    for branch, expected in expected_probs.items():
        assert np.abs(actual_probs[branch] - expected) < 0.15, f"Branch {branch} probability differs more than expected"
    ...
    for term_cell, term_name in terminal_states.items():
        ...
        assert pr_res.branch_probs.loc[term_cell, term_name] > 0.7, f"Terminal state {term_name} doesn't have high probability"
    ...
    for term_cell in terminal_states.index:
        assert pr_res.entropy[term_cell] < 0.5, f"Terminal state {term_cell} should have low entropy"
...
def test_branch_selection(processed_data):
    ...
    for term_cell, term_name in terminal_states.items():
        ...
        assert masks[ad.obs_names == term_cell, branch_idx][0], f"Terminal cell {term_name} should be selected in its branch"
