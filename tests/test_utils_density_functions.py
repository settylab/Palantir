...
def mock_anndata_with_density(mock_anndata):
    ...
    mock_anndata.layers['local_variability'] = np.random.rand(mock_anndata.n_obs, mock_anndata.n_vars)
...
def test_run_density_evaluation(mock_predictor_from_dict):
    ...
    in_ad = sc.AnnData(X=np.random.rand(20, 10))
    out_ad = sc.AnnData(X=np.random.rand(15, 10))
