...
def test_cluster_gene_trends_basic():
    ...
    for i in range(20, 30):
        trends.iloc[i] = np.sin(np.linspace(0, np.pi, n_timepoints)) + np.random.normal(0, 0.1, n_timepoints)
    ...
    assert (clusters.iloc[:10] == first_cluster).mean() > 0.5  # More than half should be in the same cluster
...
def test_cluster_gene_trends_anndata():
    ...
    n_cells = 100
    n_genes = 30
    adata = sc.AnnData(np.random.normal(0, 1, (n_cells, n_genes)))
    ...
    # Last 10 genes
    for i in range(20, 30):
        trends[i] = np.sin(np.linspace(0, np.pi, n_timepoints)) + np.random.normal(0, 0.1, n_timepoints)
    
    # Store the trends in AnnData
    adata.varm[f"gene_trends_{branch_key}"] = pd.DataFrame(
        trends, 
        index=adata.var_names,
        columns=[str(t) for t in timepoints]
    )
...
def test_cluster_gene_trends_error_handling():
    ...
    adata = sc.AnnData(np.random.normal(0, 1, (10, 10)))
